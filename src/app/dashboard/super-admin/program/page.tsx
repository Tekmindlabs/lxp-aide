...
export default function ProgramPage() {
    ...
    // Query hooks with proper error handling and loading states
    const { 
        data: calendars,
...
// Transform programs to match expected type
const transformedPrograms = programData?.programs?.map(program => ({
    id: program.id,
    ...
    coordinator: program.coordinator ? { 
        user: { 
            name: program.coordinator.user.name || 'Unknown'
        } 
    } : null,
    classGroups: program.classGroups?.map(group => ({
...
// Add error handling through toast hook
api.useContext().program.getAll.setData({ page: 1, pageSize: 10, status: "ACTIVE" }, (oldData) => {
    if (programsError) {
        toast({
            title: "Error",
